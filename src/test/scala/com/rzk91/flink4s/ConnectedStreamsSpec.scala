...
import com.rzk91.flink4s.ConnectedStreamsSpec.State
import com.rzk91.flink4s.TypeInfo.stringTypeInfo
...
final class ConnectedStreamsSpec extends AnyFunSpec with Matchers {
  ...
  describe("ConnectedStreams") {
    ...
    it("should not emit None values") {
      ...
      val results = stream.runAndCollect.groupBy(identity).view.mapValues(_.size).toMap
