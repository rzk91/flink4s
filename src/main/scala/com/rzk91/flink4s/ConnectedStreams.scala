...
import org.apache.flink.streaming.api.datastream.{ConnectedStreams => JavaCStreams}
...
final case class ConnectedStreams[A, B](streams: JavaCStreams[A, B]) {
  ...
  def biFlatMapWithState[O, S](
    ...
    val biFlatMapper: RichCoFlatMapFunction[A, B, O] with ResultTypeQueryable[O] =
      new RichCoFlatMapFunction[A, B, O] with ResultTypeQueryable[O] {
        lazy val serializer: TypeSerializer[S] =
          stateTypeInfo.createSerializer(getRuntimeContext.getExecutionConfig)
        lazy val stateDescriptor = new ValueStateDescriptor[S]("name", serializer)
        ...
        override def flatMap1(value: A, out: Collector[O]): Unit =
          map[A](value, f1).foreach(out.collect)

        override def flatMap2(value: B, out: Collector[O]): Unit =
          map[B](value, f2).foreach(out.collect)
